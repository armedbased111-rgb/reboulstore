...
let AdminJwtStrategy = class AdminJwtStrategy extends (0, passport_1.PassportStrategy)(passport_jwt_1.Strategy, 'admin-jwt') {
    configService;
    adminAuthService;
    constructor(configService, adminAuthService) {
        super({
            jwtFromRequest: passport_jwt_1.ExtractJwt.fromAuthHeaderAsBearerToken(),
            ignoreExpiration: false,
            secretOrKey: configService.get('JWT_SECRET') || 'admin-secret-key-change-in-production',
        });
    ...
    async validate(payload) {
        ...
        if (!adminUser) {
            throw new common_1.UnauthorizedException('Admin non trouvé ou compte désactivé');
        }
        return adminUser;
    }
};
