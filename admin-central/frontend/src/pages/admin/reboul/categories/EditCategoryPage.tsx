...
/**
 * Page d'édition d'une catégorie Reboul
 * 
 * Route : /admin/reboul/categories/:id/edit (protégée)
 */
export default function EditCategoryPage() {
  ...
  useEffect(() => {
    const loadCategory = async () => {
      ...
      try {
        const category = await reboulCategoriesService.getCategory(id);
        setFormData(category);
        setSizeChartItems(category.sizeChart || []);
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await reboulCategoriesService.updateCategory(id, dataToSend);
    ...
    return (
      <AdminLayout>
        <div className="text-center py-12">
          ...
          <p className="mt-4 text-sm text-gray-600">Chargement de la catégorie...</p>
        </div>
      </AdminLayout>
    );
  }
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Modifier la catégorie</h1>
