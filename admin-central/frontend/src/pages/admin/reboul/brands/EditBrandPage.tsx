...
/**
 * Page d'édition d'une marque Reboul
 * 
 * Route : /admin/reboul/brands/:id/edit (protégée)
 */
export default function EditBrandPage() {
  ...
  useEffect(() => {
    const loadBrand = async () => {
      ...
      try {
        const brand = await reboulBrandsService.getBrand(id);
        setFormData(brand);
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Erreur lors du chargement de la marque');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await reboulBrandsService.updateBrand(id, dataToSend);
      navigate('/admin/reboul/brands');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erreur lors de la mise à jour de la marque');
      setIsLoading(false);
    }
  };
    ...
    return (
      <AdminLayout>
        <div className="text-center py-12">
          ...
          <p className="mt-4 text-sm text-gray-600">Chargement de la marque...</p>
        </div>
      </AdminLayout>
    );
  }
