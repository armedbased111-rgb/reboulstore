...
import { Search, Plus, Edit, Trash2, Image as ImageIcon, Video } from 'lucide-react';
...
/**
 * Page de gestion des marques Reboul
 * 
 * Route : /admin/reboul/brands (protégée)
 * 
 * Affiche :
 * - Liste des marques avec pagination
 * - Recherche par nom
...
export default function BrandsPage() {
  ...
  const loadBrands = async () => {
    ...
    setError(null);
    try {
      const response = await reboulBrandsService.getBrands({
        page,
        limit,
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!window.confirm(`Êtes-vous sûr de vouloir supprimer la marque "${name}" ?`)) {
      return;
    }

    try {
      await reboulBrandsService.deleteBrand(id);
      loadBrands();
    } catch (err) {
      ...
      <AdminLayout>
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          <p className="text-red-700">
            Erreur lors du chargement des marques : {error.message}
          </p>
        </div>
      </AdminLayout>
    );
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">Marques Reboul</h1>
            <p className="mt-1 text-sm text-gray-600">
              Gérer les marques du site Reboul
            </p>
          </div>
          <Link
            to="/admin/reboul/brands/new"
            className={cn(
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          {loading ? (
            <div className="text-center py-12">
              ...
              <p className="mt-4 text-sm text-gray-600">Chargement des marques...</p>
            </div>
          ) : brands.length === 0 ? (
            <div className="text-center py-12">
                      ...
                      <tr key={brand.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 lg:px-6 py-4 text-right text-sm font-medium">
                          <div className="flex justify-end space-x-2">
                            <Link
                              to={`/admin/reboul/brands/${brand.id}/edit`}
                  ...
                  <div key={brand.id} className="p-4 space-y-3">
                    <div className="flex justify-between items-start">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="mt-1 text-xs text-gray-400">{brand.slug}</p>
                      </div>
                    ...
                    {brand.createdAt && (
                      <div className="text-xs text-gray-500">
                        Créé le {formatDate(brand.createdAt)}
                      </div>
                    )}
                  </div>
                ))}
