...
let ProductsController = class ProductsController {
    ...
    createImagesBulk(productId, files, body, ordersQuery) {
        ...
        const ordersArray = Array.isArray(ordersRaw)
            ? ordersRaw.map((value) => value !== undefined && value !== null && value !== ''
                ? Number(value)
                : undefined)
            : ordersRaw
                ? [
                ...
                : [];
        const createImageDtos = files.map((_, index) => ({
            alt: altsArray[index],
            order: ordersArray[index],
        }));
        return this.productsService.createImagesBulk(productId, files, createImageDtos);
    }
