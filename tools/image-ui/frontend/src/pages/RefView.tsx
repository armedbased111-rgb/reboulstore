...
import { ChevronLeft, Trash2, Wand2, Upload, Play, X, RefreshCw, Sparkles, Clapperboard, Zap, CloudUpload, Check } from 'lucide-react'
...
const ADJUST_PRESETS = [
  ...
  { label: 'Fix étiquette (ref)',  prompt: 'Using the reference photo as ground truth, fix ONLY these two things on the target image:\n1. The tongue label/tag: copy the exact text, font, layout and colors from the reference\n2. The sole: copy the exact markings, text and pattern from the reference sole\n\nDo NOT change anything else — keep the background, framing, lighting and all other shoe details identical to the target.' },
]
...
<<<<<<< HEAD
  ...
  const runGenerate = async () => {
    ...
    await api.post('/batch/start', { brand: brandName, skip_existing: false, target_refs: [refName] })
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {data.status === 'needs_generation' && (
                <button onClick={runGenerate} disabled={genRunning}
      ...
      <main className="max-w-6xl mx-auto px-8 py-6">
        ...
        {tab === 'images' && (
          !hasImages ? (
            ...
            <div className={`grid gap-6 ${selectedImg ? 'grid-cols-[1fr_320px]' : ''}`}>
              {/* Image grid */}
              <div className={`grid gap-3 ${selectedImg ? 'grid-cols-2 lg:grid-cols-3' : 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4'}`}>
                {data.images.map(img => (
                  <div
                    key={img}
                    ...
                    <div className="aspect-square bg-zinc-800 rounded-lg overflow-hidden border border-zinc-700/50">
                      <img
                        src={adjustTargetDir === 'output'
                          ...
                          : `/api/images/${encodeURIComponent(brandName)}/${encodeURIComponent(refName)}/input/${adjustTarget}`}
                        alt={adjustTarget}
                        className="w-full h-full object-contain"
                      />
                    </div>
